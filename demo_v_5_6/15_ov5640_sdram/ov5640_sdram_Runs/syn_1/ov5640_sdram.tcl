...
set rc [catch {
  open_project {ov5640_sdram.prj}
  import_device eagle_s20.db -package EG4S20BG256
  import_db {ov5640_sdram_elaborate.db}
  read_adc ../../constraints/sparkroad_large.adc
  optimize_rtl
  report_area -file ov5640_sdram_rtl.area
  export_db {ov5640_sdram_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
...
set rc [catch {
  read_sdc
  optimize_gate -maparea ov5640_sdram_gate.area
  legalize_phy_inst
  export_db {ov5640_sdram_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
