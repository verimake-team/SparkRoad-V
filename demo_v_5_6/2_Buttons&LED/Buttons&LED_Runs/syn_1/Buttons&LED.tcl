...
set rc [catch {
  open_project {Buttons&LED.prj}
  import_device eagle_s20.db -package EG4S20BG256
  elaborate -top {buttons_led}
  export_db {Buttons&LED_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../sources/sdc/button_pin.adc
  optimize_rtl
  report_area -file Buttons&LED_rtl.area
  export_db {Buttons&LED_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  read_sdc
  optimize_gate -maparea Buttons&LED_gate.area
  legalize_phy_inst
  export_db {Buttons&LED_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
